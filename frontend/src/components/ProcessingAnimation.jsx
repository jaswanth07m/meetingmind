...
const STEPS = [
  ...
  },
  {
    key: 'analyzing',
    label: 'Analyzing',
    description: 'Extracting insights, actions, and decisions',
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="w-full max-w-lg mx-auto">
        ...
        <div className="flex justify-center mb-10">
          <div className="relative">
            {step !== 'error' ? (
              <>
                <div className="w-28 h-28 rounded-3xl bg-gradient-to-br from-cyan-400 via-cyan-500 to-blue-600 flex items-center justify-center shadow-2xl shadow-cyan-500/40 ring-1 ring-white/20">
                ...
                {step !== 'done' && (
                  <>
                    <div className="absolute -inset-4 rounded-3xl border border-cyan-400/20 animate-ping" style={{ animationDuration: '2s' }} />
                    <div className="absolute -inset-8 bg-cyan-500/10 rounded-full blur-2xl" />
                  </>
                )}
              </>
            ) : (
              <div className="w-28 h-28 rounded-3xl bg-red-500/10 border border-red-500/30 flex items-center justify-center">
        ...
        {filename && (
          <div className="flex justify-center mb-5">
            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/60 border border-slate-700/50">
              ...
              <span className="text-xs text-slate-300 font-mono max-w-[220px] truncate">{filename}</span>
            </div>
          </div>
        )}
        ...
        <div className="text-center mb-8">
          {step === 'error' ? (
            ...
            <>
              <h2 className="text-2xl font-bold text-emerald-400 mb-2">Analysis Complete!</h2>
              <p className="text-slate-400 text-sm">Your meeting insights are ready.</p>
            </>
          ) : (
            <>
              <h2 className="text-2xl font-bold text-white mb-2">Processing{dots}</h2>
              <p className="text-slate-400 text-sm">Running entirely on your machine — no cloud.</p>
            </>
          )}
        </div>
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-4 p-4 rounded-2xl border transition-all duration-500 ${
                    isActive  ? 'bg-cyan-500/5 border-cyan-500/25 shadow-[0_0_20px_-8px_rgba(6,182,212,0.3)]'
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-semibold ${isActive ? 'text-cyan-300' : isDone ? 'text-emerald-300' : 'text-slate-600'}`}>
                      {s.label}
                    </p>
                    {(isActive || isDone) && (
        ...
        {step === 'uploading' && (
          <div className="space-y-2 mb-6">
            ...
            <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-cyan-500 to-blue-500 rounded-full transition-all duration-300 shadow-[0_0_8px_rgba(6,182,212,0.5)]"
